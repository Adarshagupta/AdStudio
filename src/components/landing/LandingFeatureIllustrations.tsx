...
function SectionHeading({ eyebrow, title, align = "center" }: { eyebrow: string; title: string; align?: "left" | "center" }) {
  return (
    <div className={cn("mb-8 md:mb-10", align === "center" && "mx-auto max-w-2xl text-center")}>
      <p className="text-[11px] font-semibold uppercase tracking-[0.22em] text-violet-600">{eyebrow}</p>
    ...
    <div className="flex flex-wrap items-end justify-center gap-3 py-4 md:gap-5">
      {LANDING_PREVIEW_VIDEOS.slice(0, 4).map((src, index) => (
        <LandingReveal key={src} delay={index * 70}>
          <Link
            href="/signup"
            className="landing-format-shine group relative block aspect-[9/14] w-[130px] overflow-hidden rounded-[1.35rem] border-2 border-white bg-zinc-950 shadow-[0_20px_50px_rgba(15,23,42,0.2)] transition duration-500 hover:z-10 hover:-translate-y-3 hover:scale-105 hover:shadow-[0_28px_60px_rgba(124,58,237,0.35)] sm:w-[150px] md:w-[172px]"
            style={{
              transform: `rotate(${rotations[index]}deg) translateY(${offsets[index]}px)`,
            }}
          >
            <PreviewVideo src={src} />
            <div className="absolute inset-0 bg-gradient-to-t from-violet-950/80 via-transparent to-transparent opacity-80 transition group-hover:opacity-90" />
            <div className="absolute inset-x-0 bottom-0 h-1 bg-gradient-to-r from-violet-500 via-fuchsia-400 to-cyan-400 opacity-0 transition group-hover:opacity-100" />
          </Link>
        </LandingReveal>
      ))}
    </div>
...
function BentoTile({ children, className }: { children: ReactNode; className?: string }) {
  return (
    <div
      className={cn(
        "landing-hover-lift group relative overflow-hidden rounded-[1.35rem] bg-white ring-1 ring-violet-200/40 shadow-[0_12px_40px_rgba(15,23,42,0.08)]",
        className,
      )}
    >
      {children}
...
function CornerIcon({ icon: Icon }: { icon: typeof Video }) {
  return (
    <span className="absolute left-3 top-3 z-10 flex h-9 w-9 items-center justify-center rounded-xl bg-white/90 text-violet-700 shadow-md backdrop-blur-sm transition group-hover:scale-110">
...
export function LandingFeatureIllustrations() {
  return (
    <>
      ...
      <LandingReveal className="mt-20 md:mt-28" delay={100}>
        <div id="studio" className="mb-8 flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
          ...
          <Button
            asChild
            size="lg"
            className="shrink-0 rounded-full bg-zinc-950 px-6 shadow-lg transition hover:scale-105 hover:bg-violet-950"
          >
      ...
      <LandingReveal className="mt-20 md:mt-28" delay={60}>
        ...
        <div className="grid auto-rows-[minmax(180px,auto)] gap-3 md:grid-cols-4 md:gap-4">
          <BentoTile className="md:col-span-2 md:row-span-2 min-h-[280px]">
            ...
            <div className="flex h-full min-h-[280px] flex-col justify-end gap-3 bg-gradient-to-br from-violet-50/50 to-white p-4 pt-14">
              <div className="ml-auto w-[78%] rounded-2xl rounded-tr-sm bg-violet-600 p-3 shadow-lg shadow-violet-600/25">
                <div className="space-y-2">
                  <div className="h-2 w-full rounded-full bg-white/30" />
                  <div className="h-2 w-4/5 rounded-full bg-white/20" />
                </div>
              </div>
              <div className="w-[88%] overflow-hidden rounded-2xl rounded-tl-sm bg-white p-2 shadow-xl ring-1 ring-violet-100">
          ...
          <BentoTile className="min-h-[200px]">
            ...
            <div className="flex h-full min-h-[200px] items-end gap-1.5 px-4 pb-5 pt-12">
              {BAR_HEIGHTS.map((height, index) => (
                <div
                  key={index}
                  className="landing-bar-rise flex-1 rounded-t-md bg-gradient-to-t from-violet-700 via-violet-500 to-violet-300"
